...
vector<vector<BBOX> > get_detection_result_for_video(CaffeModel& ssd_detector, string video_list, string output_list, int resize_height, int resize_width, int vis, int dete_interval, string dir) {
    cout << dir << " liuyang " << endl; 
    int show_result = vis;
...
//    }
 ...
 //   char image_filename[2000];
    ...
    int sum = 0;
    while(cur_frame < total_frame_count){
        sum++;
        cout << sum << endl;        
        Mat img;
        ...
        input_scales.push_back(make_pair(648, 1152));
        //input_scales.push_back(make_pair(1152, 648));
        ...
        for(int j = 0; j < out.size(); j++) {
            ...
            if (score > threshold[cls] && cls >= 3 && cls <= 4) {
                char char_score[100];
                ...
                if(cls >= 3 && cls <= 4 ){ //change by liuyang
                    fprintf(fid, "%.2f %.2f %.2f %.2f %.2f ",score, xmin, ymin, xmax, ymax);
                } 
        ...
        string sss = dir + "/" + int_to_str(sum) + ".jpg";
        imwrite(sss, img);
        cout << sss << endl;
        if (show_result) {
            Mat tmp = img.clone();
...
int main(int argc, char** argv) {
  ...
  vector<vector<BBOX> > detected_bboxes = get_detection_result_for_video(ssd_detector,video_list, output_list, resize_height, resize_width, vis, dete_interval, dir); 
  cout << "3" << endl;
  return 0;
}
